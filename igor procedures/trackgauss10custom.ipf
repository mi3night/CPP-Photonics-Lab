...
function trackpeak1(wpre, stnum, increment, endnum, wlen,suffix, temperature,timing,start,stop,width)
	string wpre,suffix
	wave temperature, timing,wlen
	variable stnum,endnum,increment, start, stop, width
	variable i,imax, maxloc
	variable leftminloc,rightminloc,yleft,yright,slope,intcpt
	string tempwavey,peakname,tempname,timename
	//string errname, fitname
	...
	for (i = 0; i < imax;i += 1)
		...
		//wave wcf=W_coef
		//wave wsg=W_sigma
		wavestats/Q/R=[leftminloc,rightminloc] tempwaveynorm
		maxloc=V_maxloc
		//print maxloc 
		peakuse[i]=wlen[maxloc]
		//print wlen[maxloc]
		//erruse[i]=wsg[2]
		tempuse[i]=temperature[i*increment+stnum-1]
		timeuse[i]=timing[i*increment+stnum-1]	
		killwaves tempwaveynorm
	endfor
	//timeuse=timeuse/60
	...
	//CurveFit/NTHR=0 line  peakuse[1070,2071] /X=tempuse /D
	//wave wcf=W_coef
	//fituse=wcf[0]+wcf[1]*tempuse
	//Appendtograph fituse vs tempuse
end
...
function trackgauss4a(wpre, stnum, increment, endnum, wlen,suffix, temperature,timing,start,stop,width) //with normalization, also plots peak vs temp
	...
	variable stnum,endnum,increment, start, stop, width
	variable i,imax,maxloc,leftminloc,rightminloc,yleft,yright,slope,intcpt
	string tempwavey,peakname,errname,tempname,timename,fitname
	imax=floor((endnum-stnum)/increment)+1
	peakname="peak"+suffix
	errname="err"+suffix
	tempname="temp"+suffix
...
function trackgaussall4a(wpre,stnum,increment,endnum, wlen,temperature, timing,startpeak,stoppeak,width) //analyzes all peaks
	string wpre
	wave temperature, timing,wlen,startpeak,stoppeak,width
	variable stnum,endnum,increment
	variable i,len
	string suffixi
	//wavestats/Q startpeak
	wavestats startpeak
	len= V_npnts
	for  (i = 0; i < len;i += 1)
	suffixi=num2str(round((wlen[startpeak[i]]+wlen[stoppeak[i]])/2))
	//trackgauss4a(wpre, stnum, increment, endnum, wlen,suffixi, temperature,timing,startpeak[i]-round(0.2*width[i]),stoppeak[i]+round(0.2*width[i]),width[i])
	trackgauss4a(wpre, stnum, increment, endnum, wlen,suffixi, temperature,timing,startpeak[i]-3,stoppeak[i]+3,width[i])
	endfor
end
